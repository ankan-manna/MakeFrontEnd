import { Card } from '../../components/ui/Card'
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts'

const placeholderData = [
  { name: 'Mon', value: 120 },
  { name: 'Tue', value: 118 },
  { name: 'Wed', value: 125 },
  { name: 'Thu', value: 122 },
  { name: 'Fri', value: 119 },
  { name: 'Sat', value: 121 },
  { name: 'Sun', value: 117 },
]

export function PatientRemoteMonitoring() {
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-bold text-neutral-900">Remote Monitoring</h1>
        <p className="text-sm text-neutral-500 mt-1">Track vitals from your connected devices</p>
      </div>

      <div className="grid sm:grid-cols-3 gap-4">
        <Card>
          <p className="text-sm text-neutral-500">Blood Pressure</p>
          <p className="text-2xl font-bold text-neutral-900 mt-1">120/80</p>
          <p className="text-xs text-emerald-600 font-medium mt-1">Normal range</p>
        </Card>
        <Card>
          <p className="text-sm text-neutral-500">Heart Rate</p>
          <p className="text-2xl font-bold text-neutral-900 mt-1">72 bpm</p>
          <p className="text-xs text-emerald-600 font-medium mt-1">Normal range</p>
        </Card>
        <Card>
          <p className="text-sm text-neutral-500">Glucose</p>
          <p className="text-2xl font-bold text-neutral-900 mt-1">95 mg/dL</p>
          <p className="text-xs text-emerald-600 font-medium mt-1">Normal range</p>
        </Card>
      </div>

      <Card>
        <h3 className="font-semibold text-neutral-900 mb-1">Blood Pressure Trend</h3>
        <p className="text-sm text-neutral-500 mb-6">Systolic readings over the past week (sample data)</p>
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={placeholderData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
              <XAxis dataKey="name" tick={{ fontSize: 13, fill: '#64748b' }} />
              <YAxis tick={{ fontSize: 13, fill: '#64748b' }} />
              <Tooltip
                contentStyle={{
                  backgroundColor: '#fff',
                  border: '1px solid #e2e8f0',
                  borderRadius: '12px',
                  boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)',
                  fontSize: 13,
                }}
              />
              <Bar dataKey="value" fill="#06c4af" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Card>
    </div>
  )
}
