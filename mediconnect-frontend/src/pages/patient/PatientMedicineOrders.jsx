...
export function PatientMedicineOrders() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-extrabold text-neutral-900">Medicine Orders</h1>
      ...
      <Card>
        {orders.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="font-semibold text-neutral-700 mb-1">No orders yet</p>
            <p className="text-sm text-neutral-400">Orders are created when a prescription is sent to the pharmacy.</p>
          </div>
        ) : (
          <ul className="divide-y divide-neutral-100">
            {orders.map((o) => (
              <li key={o.id} className="py-4 flex flex-wrap items-center justify-between gap-3 hover:bg-neutral-50/50 px-1 -mx-1 rounded-xl transition-soft">
